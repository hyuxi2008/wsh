...
static void
move_chars_right (char *line, int pos, int end)
{
...
static void
move_chars_left (char *line, int pos, int end)
{
  //assert(pos);
...
static void
update_line (history_entry_t* line)
{
...
static void
copy_history_to_current_line (void)
{
...
key_delete (void)
{
  ...
  if (cur_pos != the_line->end) {
    ...
    move_chars_left(the_line->line, cur_pos+1, the_line->end);
    the_line->end--;
  }
  ...
  while (1) {
    ...
    if (cur_edit_mode == edit_mode_overwrite) {
      ...
      if (cur_pos == the_line->end) {
        ...
        wsh_printf("\e[1@%c", c);
        move_chars_right(the_line->line, cur_pos, the_line->end);
...
int
wsh_cmd_history (int argc, char** argv)
{
